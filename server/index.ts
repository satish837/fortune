...
import { handleGenerate, handleTestClipdrop, handleTestFluxKontext } from "./routes/generate";
...
export function createServer() {
  ...
  app.get("/api/test-env", (req, res) => {
    const envVars = {
      ...
      clipdropApiKeyPreview: process.env.REMOVE_BG_API_KEY ? process.env.REMOVE_BG_API_KEY.substring(0, 8) + '...' : 'Not set',
      tinyPNGApiKeyPreview: process.env.TINYPNG_API_KEY ? process.env.TINYPNG_API_KEY.substring(0, 8) + '...' : 'Not set'
    };
    res.json({ 
  ...
  app.post("/api/test-bg-removal", async (req, res) => {
    try {
      ...
      if (!clipdropResponse.ok) {
        ...
        throw new Error(`Clipdrop API failed: ${clipdropResponse.status} ${clipdropResponse.statusText} - ${error}`);
      }
      ...
      cloudinaryForm.append("file", new Blob([backgroundRemovedBuffer as any]), "image.png");
      ...
      if (!cloudinaryRes.ok) {
        ...
        throw new Error(error?.error?.message || `Cloudinary ${cloudinaryRes.status}`);
      }
