...
export const handleOptimizeImage: RequestHandler = async (req, res) => {
  try {
    ...
    if (!tinifyRes.ok) {
      ...
      return res.status(500).json({ error: 'TinyPNG shrink failed', details: text });
    }
    ...
    if (!optimizedRes.ok) {
      ...
      console.error('Failed to download optimized image:', optimizedRes.status, text);
      return res.status(500).json({ error: 'Failed to download optimized image', details: text });
    }
